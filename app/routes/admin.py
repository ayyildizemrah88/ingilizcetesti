    ...
    try:
        ...
        if sirket_ids:
            for sirket_id in sirket_ids:
                sirket = Sirket.query.get(sirket_id)
                if sirket:
...
def aday_sinav_sifirla(id):
    ...
    try:
        from app.models import Aday, SinavCevap
        from app import db
        
        aday = Aday.query.get_or_404(id)
        # Sınav cevaplarını sil
        SinavCevap.query.filter_by(aday_id=id).delete()
...
def toplu_aday_sil():
    ...
    try:
        ...
        if aday_ids:
            Aday.query.filter(Aday.id.in_(aday_ids)).delete(synchronize_session=False)
            db.session.commit()
